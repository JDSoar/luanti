...
fake_function() {
	...
	gettext("Gamepad");
	gettext("Enable gamepads");
	gettext("Enable gamepad / joystick input. A restart is required after changing this.");
	gettext("Primary gamepad device index");
	gettext("Which physical controller is player 1 in single-player (first device is 0).\nIn couch split-screen with multiple local players, devices 0 to 3 map to\nplayers 1 to 4 in order; this setting then only affects solo play.");
	gettext("Gamepad layout preset");
	gettext("Button and axis layout preset. \"auto\" picks from the detected device name.\nUse \"generic\" for the classic Luanti layout, or match your controller type.");
	gettext("Gamepad button repeat interval");
	gettext("Minimum time in seconds between repeated actions while holding a gamepad\nbutton combination (for keys that support repeat).");
	gettext("Gamepad analog dead zone");
	gettext("Analog stick dead zone. Larger values ignore more small stick movement.");
	gettext("Gamepad look sensitivity");
	gettext("Sensitivity of the look (right) stick for turning the camera view.");
}
